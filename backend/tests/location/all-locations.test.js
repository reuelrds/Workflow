...
/**
 * Testing Get all-locations Route
 */
describe('Testing get all-locations Endpoint', () => {
 ...
 // For CheckAuth Purpose
  ...
  test('it should fetch a list containing all locations', async () => {
    const result = await request(app)
      .get(`/api/location/all-locations`)
      .set('Authorization', 'Bearer ' + token)
      .expect(200);

    expect(result.body.locations).toMatchObject([dbSetup.locationOne]);
  });
