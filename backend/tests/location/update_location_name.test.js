...
/**
 * Testing Update Location Name Route
 */
describe('Testing updateLocationName Endpoint', () => {
 ...
 // For CheckAuth Purpose
  ...
  test('it should update a new location Name', async () => {
    const res = await request(app)
      .patch(`/api/location/${dbSetup.locationOne.id}/updateLocationName`)
      .set('Authorization', 'Bearer ' + token)
      .send({
        newLocationName: "Jest"
      })
      .expect(200);

    expect(res.body.message).toContain("Updated Office Location Name");
    expect(res.body.location.locationName).toEqual("Jest");
  });
  
  test('it should throw an error if no location is found', async () => {
    try {
      await request(app)
        .patch(`/api/location/invalidId/updateLocationName`)
        .set('Authorization', 'Bearer ' + token)
        .send({
          newLocationName: "Test Id"
        });
    } catch (error) {
      console.log(error);
