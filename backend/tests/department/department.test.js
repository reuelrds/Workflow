...
/**
 * Testing Get Department Route
 */
describe('Testing get Department Endpoint', () => {
 ...
 // For CheckAuth Purpose
  ...
  test('it should fetch Department details', async () => {
    const res = await request(app)
      .get(`/api/department/${dbSetup.departmentOne.id}`)
      .set('Authorization', 'Bearer ' + token)
      .expect(200);

    expect(res.body.department).toMatchObject(dbSetup.departmentOne);
  });

  test('it should throw an error if an invalid id is passed', async () => {
    try {
      await request(app)
        .get(`/api/department/error`)
        .set('Authorization', 'Bearer ' + token)
      
    } catch (error) {
      expect(error.status).toBe(404);
      expect(error.response.body.message).toContain('Department Data Not Found');
    }
  });
